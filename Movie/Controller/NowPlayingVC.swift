...
class NowPlayingVC: UIViewController, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func getMovies(forPage: String, handler: @escaping (_ success: Bool)->()){
        Alamofire.request("\(GET_NOWPLAYING_URL)\(forPage)").responseJSON { response in
